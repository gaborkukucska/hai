...
## NoSlop Integration & Identity Cloning (July 2026)
*   **Active-Passive Mesh Topology:** We successfully transitioned NoSlop from a standalone mesh node into an active-passive client for a HAI-Net Hub. The mobile device provisions the Hub via SSH, injecting its exact Ed25519 private key. 
*   **Native Tor Key Expansion:** Android's KeyStore generates Ed25519 keys in standard PKCS#8 format. To migrate this identity to a standard Linux `tor` daemon without Java, we implemented native Rust expansion in `hainet-seed`. The Rust installer decodes the PKCS#8 Base64 string, extracts the 32-byte seed, expands it via SHA-512, applies standard Ed25519 bit-clamping, and writes it directly to `/var/lib/tor/hainet/hs_ed25519_secret_key`. This guarantees the Hub mathematically derives the exact same `.onion` address as the mobile phone, seamlessly taking over the user's mesh presence.
*   **Unified Port Routing:** By binding both `9999` (Mesh Gossip) and `8080` (API Portal) to the same Hidden Service in `/etc/tor/torrc`, we allow the mobile app to securely sync data from anywhere in the world using a single `.onion` address.
*   **Intelligent Edge Relay vs Dumb Buffer:** Initially, we considered making the Hub blindly buffer all Port 9999 Tor packets to pass to the mobile app. We realized this defeated the purpose of an always-on edge node and exposed the mobile app to spam. Instead, the mobile app pushes its `Trusted Peers` list to the Hub (`sync_push_peers`). The Hub runs its native `GossipEngine` and `TrustFirewall`, discarding spam and expired TTLs *before* they ever reach the phone, and only buffers valid packets for the mobile app to pull (`sync_pull_packets`).
*   **The "Empty Portal" Phenomenon:** Because Phase 2 focuses purely on routing, the Hub's firewall currently holds valid packets in volatile memory (`RwLock<Vec<Value>>`) until the mobile app pulls them. Consequently, the `hainet-portal` Web UI has no persistent data to display. Phase 3 will introduce a Rust SQLite schema to mirror the mobile device's state, populating the Web UI.
